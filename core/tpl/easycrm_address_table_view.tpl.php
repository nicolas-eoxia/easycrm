...
if (is_array($addresses) && !empty($addresses)) {
	foreach ($addresses as $element) {
		...
		// Address name
		print '<td>';
		print $contact->getNomUrl(1) . ' ' . ($permissiontoadd ? '<span style="cursor:pointer;" name="favorite_address" id="address"' . $element['id'] . ' value="' . $element['id'] . '" class="' . ($favorite ? 'fas' : 'far') . ' fa-star"></span>' : '');
		print '</td>';
